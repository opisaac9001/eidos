...
| Memory is recalled or explicitly prompted by the user | `memory.accessed`, optionally `memory.reminded` and `memory.reconsolidated` | Separately capped reinforcement, attention, mood-congruent selection, selective drift, and source-confused content/person/place/time attribution | Actor can access every cited source; changed attribution and time must come from a cited companion memory, future timestamps are rejected, reminders never become another occurrence, and immutable evidence stays separate |
...
| Repeated remembered encounters imply a pattern | `semantic.expectation_formed`, `semantic.expectation_reinforced`, or `semantic.expectation_revised` | Conversation and feasible planning context, operator memory view | At least three Pathos-owned non-dream sources on distinct subjectively remembered days support the same person/place pair; the expectation remains explicitly fallible and never proves current presence |
| Repeated outcomes shape Pathos's current self-story | `self_concept.formed`, `self_concept.reinforced`, or `self_concept.revised` | Conversation, reflection, activity and project motivation, operator identity view | Three or more Pathos-owned resolved outcomes span at least a week; revisions wait fourteen days, require new evidence, preserve prior versions, and never become fixed traits or world truth |
| Lived learning or repair changes capability | `skill.practiced` or `skill.rusted` | Activity and project proposals, repair feasibility, Observatory | Practice cites a matching validated action or realized learning event; gains diminish with level, rust waits ninety days and changes at most monthly, and capability never grants action authority or guarantees success |
| Repeated voluntary activity becomes a familiar rhythm | `habit.formed`, `habit.reinforced`, `habit.lapsed`, or `habit.reactivated` | Activity and project proposals, reflection, Observatory | Realized actions match activity/place/time context on distinct days across a week; changes wait fourteen days, disuse lasts forty-five days before lapse, and the habit remains a soft influence with no action authority |
| A model keeps proposing the same voluntary activity | `agency.activity_proposed` then `agency.activity_rejected` with `overused_pattern` | Agency audit and later proposal context | The same activity/place/company combination already occurred six times, or the activity meaning occurred eight times, within the bounded recent context; no schedule, intention, or accomplishment is created |
