...
| Memory is recalled | `memory.accessed`, optionally `memory.reconsolidated` | Retrieval reinforcement, attention, selective subjective drift | Actor can access it; one bounded update per meaningful access, with immutable evidence kept separate |
