...
| Models from any provider | Real models are set up on the operator's Models page, or in `~/.config/eidos/models.json` (`EIDOS_MODELS_FILE`): providers, models, per-role first choices and backups, and a daily budget. It applies without a restart; with no models, the offline stand-in runs as before. The old environment variables and routing file still take precedence when set. New role `pathos_voice` (routed like `reflection` unless named) re-tells narrated memories in his words (`authored_text` keeps the original). See `docs/MODELS.md`. |
