...
| PLAN-05 | Interrupt, postpone, renegotiate, cancel, bounded replan | Partial: interruption, reflection-triggered feasible two-party promise retiming with delayed independent creditor response + atomic project cancellation | P3 |
...
| WORLD-07 | Offscreen low-detail simulation, scene activation, pacing | Partial: explicit background/local/focused cognitive LOD is cheaply derived from route distance, co-presence, scenes, and Pathos attention without per-NPC bookkeeping events; all tiers retain bounded hourly movement and six-hour consequences | P5 |
...
| INNER-03 | Reflection proposes interpretations, links, concern/plan updates | Partial: continuous deliberative focus plus source-linked scheduled interpretation whose bounded emotional echo follows the appraised lived source rather than model prose; reflection on an appraised setback can raise an expiring typed planning question, later agency may schedule actual time to reconsider, and completing that review yields a linked typed decision; only an unobligated blocked personal goal may be released, through the existing abandonment resolver | P4 |
