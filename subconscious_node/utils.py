...
LLAMA_MODEL_NAME = "default_model_name" # Added a default
DEMO_MODE = False
...
try:
    if os.path.exists(CONFIG_FILE_PATH):
        ...
        LLAMA_MODEL_NAME = llm_settings.get("llama_model_name", LLAMA_MODEL_NAME) # Load model name
        ...
        logger.info(f"Llama.cpp settings loaded: URL='{LLAMA_CPP_SERVER_URL}', Model='{LLAMA_MODEL_NAME}', MaxTokens={LLAMA_CPP_MAX_TOKENS}, DefaultTemp={LLAMA_CPP_DEFAULT_TEMP}, DemoMode={DEMO_MODE}")
...
def run_llm(prompt: str, temperature: Optional[float] = None) -> str:
    ...
    payload = {
        "model": LLAMA_MODEL_NAME, # Added model name to payload
    ...
    full_content = []
    try:
        # Added stream=True to the requests.post call
        response = requests.post(LLAMA_CPP_SERVER_URL, json=payload, timeout=180, stream=True)
        response.raise_for_status()
        
        print("\\nStreamed thought: ", end='', flush=True) # Indicate start of streaming
            ...
            if line:
                ...
                if decoded_line.startswith("data: "):
                    ...
                    try:
                        ...
                        logger.error(f"Error decoding JSON chunk: {json_content}")
        ...
        if full_content:
            ...
            # This case might occur if the stream ends without any content parts
            # or if there was an issue not caught by raise_for_status but resulted in no valid chunks
            logger.warning(f"LLM stream ended without producing content. Full response status: {response.status_code}")
            return "A quiet moment of reflection passes through the mind, but no words form."
