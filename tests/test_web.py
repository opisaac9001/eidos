...
class WebTests(unittest.TestCase):
    ...
    def test_live_loop_runs_pauses_and_serves_assets(self):
        ...
        for path in ("/", "/operator", "/app.js", "/style.css", "/health", "/api/export"):
            ...
            if path == "/":
                ...
                self.assertIn(b'id="emotion-chart"', body)
                self.assertIn(b'id="emotion-influences"', body)
                self.assertIn(b'id="ahead-in-mind"', body)
            ...
            if path == "/app.js":
                ...
                self.assertIn(b"function renderAheadInMind()", body)
                self.assertIn(b"A SMALL PLAN SLIPPED HIS MIND", body)
    ...
    def test_live_visit_and_immediate_exchange_work_through_http(self):
        ...
        status, body = self.request(
            "POST", "/api/chat", {"text": "And one more thing", "request_id": "http-live-2"}
        )
        self.assertEqual(status, 400)
        self.assertEqual(json.loads(body)["error"], "Pathos is still speaking")
