...
def project_semantic_expectations(events: Sequence[DomainEvent]) -> SemanticExpectationState:
    ...
    person_attribution: dict[str, str | None] = {}
    location_attribution: dict[str, str | None] = {}
    remembered_times: dict[str, datetime] = {}
    felt_confidences: dict[str, float] = {}
    ...
    for event in events:
        if event.kind == "memory.recorded" and event.payload.get("owner", "pathos") == "pathos":
            ...
            sources[memory_id] = event
            person_attribution[memory_id] = _optional_string(event, "person_id")
            location_attribution[memory_id] = _optional_string(event, "location_id")
            remembered_times[memory_id] = _event_time(event)
            felt_confidences[memory_id] = _confidence(event)
            ...
            person_attribution[memory_id] = _optional_string(
                event, "remembered_person_id"
            ) or _optional_string(source, "person_id")
            location_attribution[memory_id] = _optional_string(
                event, "remembered_location_id"
            ) or _optional_string(source, "location_id")
            remembered_times[memory_id] = _optional_time(event, "remembered_at") or _event_time(
                source
            )
            felt_confidences[memory_id] = _confidence(event)
            ...
            if source is None:
                continue
            memory_id = str(raw_memory_id)
            person_attribution[memory_id] = _optional_string(source, "person_id")
            location_attribution[memory_id] = _optional_string(source, "location_id")
            remembered_times[memory_id] = _event_time(source)
            felt_confidences[memory_id] = _confidence(event)
        elif event.kind in {
            ...
            if prior is not None and (
                prior.subject_id != subject_id or prior.predicate != predicate
            ):
            ...
            latest[expectation_id] = SemanticExpectation(
                expectation_id,
                revision,
                subject_id,
                predicate,
    ...
    return SemanticExpectationState(latest)
