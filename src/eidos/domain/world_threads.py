...
class _ThreadFold(NamedTuple):
    threads: Mapping[str, WorldThread]
    latest_transition: Mapping[str, str]
    seen: PersistentMap[str, DomainEvent]


_THREAD_KINDS = frozenset(
    {
        "world_thread.opened",
...
def _thread_step(fold: _ThreadFold, event: DomainEvent) -> _ThreadFold:
    seen = fold.seen
    if event.kind not in _THREAD_KINDS:
        return fold._replace(seen=seen.with_item(str(event.event_id), event))
    # Thread transitions are rare; copying the small maps keeps earlier states immutable.
    threads = dict(fold.threads)
    latest_transition = dict(fold.latest_transition)
    if event.kind == "world_thread.opened":
        ...
        midpoint = (
            ...
            + (datetime.fromisoformat(thread.due_at) - datetime.fromisoformat(thread.started_at))
            / 2
        )
        ...
        latest_transition[thread_id] = str(event.event_id)
    return _ThreadFold(
        MappingProxyType(threads),
        MappingProxyType(latest_transition),
        seen.with_item(str(event.event_id), event),
    )


_THREAD_FOLD: IncrementalFold[_ThreadFold] = IncrementalFold(
    lambda: _ThreadFold(MappingProxyType({}), MappingProxyType({}), PersistentMap()),
    _thread_step,
)
...
def project_world_threads(history: Sequence[DomainEvent]) -> dict[str, WorldThread]:
    ...
    return dict(_THREAD_FOLD(history).threads)
