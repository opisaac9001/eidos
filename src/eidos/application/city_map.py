...
def city_map(history, catalog, current_location_id):
    ...
    return {
        "places": {
            place_id: {
                "experience": "here" if place_id == current_location_id else
                    "visited" if visits[place_id] else "known_not_visited",
        ...
        "routes": [
            ...
            for edge, minutes in sorted(catalog.route_minutes.items(), key=lambda item: sorted(item[0]))
        ],
    }
