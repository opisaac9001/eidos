...
function renderArchive() {
  ...
  $("memory-list").innerHTML = items.length
    ? items
        .map(
          (item) =>
            `<article class="memory-card"><div class="memory-meta"><span>${esc(date(item.simulated_at))} · ${esc(time(item.simulated_at))}</span><span>${item.archived ? "COLD ARCHIVE · " : ""}${esc((item.category || "experience").toUpperCase())} · ${esc((item.detail_level || "clear").toUpperCase())} · ${Math.round((item.accessibility ?? 1) * 100)}% ACCESSIBLE</span></div><p>${esc(item.recalled_text || item.text)}</p><div class="memory-source">${esc(item.source || "authored-routine")}${item.source_event_id ? ` · linked to event ${esc(item.source_event_id.slice(0, 8))}` : ""}${(item.emotional_intensity || 0) >= 0.15 ? ` · FELT ${esc((item.emotional_label || "quiet").toUpperCase())} (${Math.round(item.emotional_intensity * 100)}%)` : ""}${item.recalled_text && item.recalled_text !== item.text ? " · SUBJECTIVE RECOLLECTION; SOURCE PRESERVED" : ""}${item.correction_evidence_id ? ` · CORRECTED FROM DIRECT EVIDENCE ${esc(item.correction_evidence_id.slice(0, 8))}` : ""}${Math.abs(item.affective_bias || 0) >= 0.25 ? " · MOOD-COLORED WHEN RECALLED" : ""}${(item.blended_memory_ids || []).length ? ` · BLENDED WITH ${item.blended_memory_ids.length} RELATED MEMORY` : ""}${item.archived ? " · ORIGINAL EVIDENCE RETAINED" : ""}</div></article>`,
        )
        .join("")
