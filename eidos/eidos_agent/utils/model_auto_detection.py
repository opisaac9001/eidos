...
class ModelAutoDetector:
    ...
    async def detect_model_for_config(self, llm_config: LLMConfig) -> Optional[str]:
        ...
        timeout = float(llm_config.get("timeout", 5.0))  # Reduced from 10.0 to 5.0 for faster startup
...
async def get_available_models_for_url(url: str, api_key: Optional[str] = "lm-studio", timeout: float = 5.0) -> List[str]:  # Reduced from 10.0 to 5.0
