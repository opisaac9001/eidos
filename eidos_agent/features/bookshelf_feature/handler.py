...
class BookshelfHandler:
    ...
    async def add_document_to_bookshelf(
        ...
        try:
            # Add to RAG client (vector DB)
            processed_chunks = self.bookshelf_client.add_document(
                doc_id=doc_id,
                content=content_to_process,
                metadata=client_metadata # Pass metadata to be stored with chunks
            )

            if not processed_chunks:
                logger.warning(f"RagbitsBookshelfClient returned no processed chunks for doc_id '{doc_id}'. Document might not have been added effectively.")
                # Depending on desired behavior, could return None or error here.

            # Add to MemoryStorage (index record)
            index_entry_content_typed = BookshelfDocumentIndex( # Use TypedDict for structure
                doc_id=doc_id,
                title=final_title,
                original_source=original_source_for_index,
                user_id=user_id,
                added_timestamp=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                client_doc_metadata=client_metadata, # Store the metadata passed to client
                chunk_count=len(processed_chunks) if processed_chunks else 0
            )
            ...
            logger.info(f"Document '{final_title}' (ID: {doc_id}) added to bookshelf for user '{user_id}'. Indexed with {len(processed_chunks) if processed_chunks else 0} chunks.")
            return {"doc_id": doc_id, "title": final_title, "chunks_count": len(processed_chunks) if processed_chunks else 0}

        except Exception as e:
            logger.error(f"Error adding document '{final_title}' to bookshelf: {e}", exc_info=True)
            # Consider if cleanup is needed in RAG client if indexing fails partway
            return None

    async def query_bookshelf(self, user_id: str, query_text: str, top_k: int = 3) -> List[Dict[str, Any]]:
        logger.info(f"Querying bookshelf for user '{user_id}' with query: '{query_text[:50]}...'")

        filter_for_query = {"user_id": user_id}

        results = self.bookshelf_client.query_bookshelf(
            query_text=query_text,
            top_k=top_k,
            filter_conditions=filter_for_query
        )
    ...
    async def get_document_raw_text(self, user_id: str, doc_id: str) -> Optional[str]:
        ...
        chunks_data = self.bookshelf_client.get_document_chunks(doc_id=doc_id)
        if not chunks_data:
            logger.warning(f"No chunks found for document ID '{doc_id}' in RAG client.")
            return None

        full_text_parts = [chunk.get("text", "") for chunk in chunks_data]
        full_text = "".join(full_text_parts)

        logger.info(f"Retrieved and reassembled raw text for document ID '{doc_id}'. Length: {len(full_text)}")
        return full_text
    ...
    async def remove_document_from_bookshelf(self, user_id: str, doc_id: str) -> bool:
        ...
        delete_success_rag = self.bookshelf_client.delete_document_chunks(doc_id=doc_id)
...
if __name__ == '__main__':
    ...
    async def run_handler_tests():
        ...
        added_doc_1_info = await handler.add_document_to_bookshelf(
            user_id=test_user_id,
            file_path_or_url="test_doc_1.txt", # Used for title if not provided
            document_content=doc_content,
            document_title="My First Test Document",
            source_metadata={"category": "testing", "version": "1.0"}
        )
        doc1_id_val = None # Initialize
        if added_doc_1_info and "doc_id" in added_doc_1_info:
            print(f"Added document 1 successfully: {added_doc_1_info}")
            doc1_id_val = added_doc_1_info["doc_id"]

            # Test listing documents
            print("\n2. Listing documents...")
            docs_list = await handler.list_documents_on_bookshelf(test_user_id)
            ...
            query_results = await handler.query_bookshelf(test_user_id, "direct content provision", top_k=2)
            ...
            raw_text = await handler.get_document_raw_text(test_user_id, doc1_id_val)
            if raw_text: print(f"  Retrieved raw text (first 100 chars): '{raw_text[:100]}...'")
            else: print(f"  Failed to retrieve raw text for {doc1_id_val}")

        else:
            print(f"Failed to add document 1. Info: {added_doc_1_info}")

        # Test adding a document via URL (placeholder)
        print("\n5. Adding document via URL (placeholder)...")
        added_doc_2_info = await handler.add_document_to_bookshelf(
            user_id=test_user_id,
            file_path_or_url="http://example.com/testpage_for_handler", # Unique URL
            is_url=True,
            source_metadata={"source_type": "web_test"}
        )
        if added_doc_2_info and "doc_id" in added_doc_2_info:
             print(f"Added document 2 (URL) successfully: {added_doc_2_info}")
        else:
            print(f"Failed to add document from URL. Info: {added_doc_2_info}")

        # Test removal if doc1 was added
        if doc1_id_val:
            print(f"\n6. Removing document ID: {doc1_id_val}...")
            remove_success = await handler.remove_document_from_bookshelf(test_user_id, doc1_id_val)
            print(f"  Removal successful: {remove_success}")

            docs_list_after_remove = await handler.list_documents_on_bookshelf(test_user_id)
    ...
    bookshelf_client_instance.close() # Close client after tests
    memory_storage_instance.close_connection() # Close memory after tests
