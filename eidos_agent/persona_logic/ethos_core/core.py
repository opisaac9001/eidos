    ...
    async def run_hexus_decay(self):
        ...
        for key, current_value in list(self.hexus_scores.items()): # Iterate over a copy if modifying
            ...
            if abs(clamped_new_value - current_value) > 1e-4: # Only update if change is significant
                ...
                changed_scores = True
                logger.debug(f"Hexus decay for '{key}': {current_value:.3f} -> {clamped_new_value:.3f} (baseline: {effective_baseline:.2f}, rate/hr: {effective_rate_per_hour:.3f}, dt: {time_elapsed_seconds:.0f}s). Change: {decay_amount_for_cycle:+.4f}")
