...
class EthosConfig(TypedDict, total=False):
    ...
    retrieval_min_salience_for_pathos_context: float # Added for prompt_builder context retrieval
...
class Config:
    LLM: Dict[str, LLMConfig] = {
        "PATHOS": {
            ...
            "timeout": float(os.getenv("LLM_PATHOS_TIMEOUT", 10.0)),  # Reduced from 300.0 to 10.0 for faster startup
            ...
            "supports_vision": os.getenv("LLM_PATHOS_SUPPORTS_VISION", "False").lower() == "true", # Read from .env            "model_name_for_tiktoken": os.getenv("LLM_PATHOS_TIKTOKEN_NAME", "cl100k_base")
        },
        "LOGOS_TECHNE": {
            ...
            "timeout": float(os.getenv("LLM_LOGOS_TECHNE_TIMEOUT", 10.0)),  # Reduced from 300.0 to 10.0 for faster startup
        ...
        "LOGOS_DEEP_RESEARCH": {
            ...
            "timeout": float(os.getenv("LLM_LOGOS_RESEARCH_TIMEOUT", 10.0)),  # Reduced from 180.0 to 10.0 for faster startup
    ...
    ETHOS: EthosConfig = {
        ...
        "retrieval_min_salience_for_pathos_context": float(os.getenv("ETHOS_RETRIEVAL_MIN_SALIENCE_PATHOS", "0.1")), # Added
    }
